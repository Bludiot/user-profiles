...
class Simple_Image {
	 ...
	 *         file|URI or image data is invalid.
	 * @return self
	 */
	...
	public function fromFile( $file ) {
		 ...
		 * Uses fopen() instead of file_exists() because
		 * not all URL wrappers support the latter.
		 */
		$handle = @fopen( $file, 'r' );
		if ( $handle === false ) {
		...
		switch ( $this->mimeType ) {
			case 'image/gif':
				...
				if ( $gif ) {
					/**
					 * Copy the gif over to a true color image to
					 * preserve its transparency.
					 *
					 * This is a workaround to prevent imagepalettetruecolor()
					 * from borking transparency.
					 */
					$width  = imagesx( $gif );
					$height = imagesy( $gif );
		...
		// Convert pallette images to true color images.
	 ...
	 * Class from string
	 *
	 * @example $string = file_get_contents( 'image.jpg' );
	 *
	 * @since  1.0.0
	 * @access public
	 ...
	 * Class to download
	 *
	 * Forces the image to be downloaded to the clients machine.
	 ...
	 * Class to screen
	 *
	 * Outputs the image to the screen. Must be called
	 * before any output is sent to the screen.
	 *
	 * @since  1.0.0
	 * @access public
	 * @param  string $mimeType The image format to output as a mime type (defaults to the original mime type).
	 ...
	 * Auto orient
	 *
	 * Rotates an image so the orientation will be correct
	 * based on its exif data. It is safe to call this method
	 * on images that don't have exif data (no changes will be made).
	 *
	 * @since  1.0.0
	 * @access public
	 * @return self
	 ...
	 * Best fit
	 *
	 * Proportionally resize the image to fit
	 * inside a specific width and height.
	 *
	 * @since  1.0.0
	 * @access public
	 ...
	 * @since  1.0.0
	 * @access public
	 * @param  string|Simple_Image $overlay The image to overlay. This can be a filename, a data URI, or a Simple_Image object.
	...
	public function overlay( $overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0, $calculateOffsetFromEdge = false ) {
		...
		// Load overlay image
		if ( ! ( $overlay instanceof Simple_Image ) ) {
			$overlay = new Simple_Image( $overlay );
		}
	 ...
	 * If only one dimension is specified, the image
	...
	public function resize( $width = null, $height = null ) {
		...
		/**
		 * We can't use imagescale because it doesn't seem to
		 * preserve transparency properly. The workaround is to
		 * create a new truecolor image, allocate a transparent color,
		 * and copy the image over to it using imagecopyresampled.
		 */
	 ...
	 * @param  string|array $backgroundColor The background color to
	 *                      use for the uncovered zone area after
	 *                      rotation (default 'transparent').
	 * @return self
	 */
	 ...
	 * @param s tring|array $borderColor The color of border.
	 * @return self
	 */
	...
	public function extractColors( $count = 5, $backgroundColor = null ) {
		...
		if ( ! class_exists( '\League\ColorExtractor\ColorExtractor' ) ) {
			throw new \Exception( 'Required library \League\ColorExtractor is missing.', self :: ERR_LIB_NOT_LOADED );
		}
