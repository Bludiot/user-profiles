...
class Avatar_Images_Helper {
	...
	public function adminJSData( $domainPath ) {
		...
		<script>
			var avatar = {
			config : {
				ajaxUrl : "' . $domainPath . 'ajax/avatar-request-handler.php"
			},
			L: {
	...
	public function dropzoneJSData( $album ) {
		...
		$upload  = "<strong>{$L->get( 'Upload site icons here' )}</strong>";
		...
		Dropzone.options.avatarUpload = {
			...
			init : function() {
				this.on( "queuecomplete", function() {
					$( "#avatar-upload-notice" ).fadeIn( 250 );
				});
