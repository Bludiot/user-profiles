...
class Cover_Images {
	 ...
	 * @var    string
	 */
	protected $path_thumbnail = 'cache' . DS . 'thumb' . DS;
	 ...
	 * @var    string
	 */
	protected $path_large = 'cache' . DS . 'large' . DS;
	...
	public function images( $album, $sort = 'default' ) {
		...
		foreach ( $list as $image => $timestamp ) {
			...
			$images[$image]['thumbnail'] = $this->urlPath( $album ) . $this->path_thumbnail . $image;
			$images[$image]['large']     = $this->urlPath( $album ) . $this->path_large . $image;
		}
		return $images;
	}
}
